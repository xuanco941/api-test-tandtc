...
export class AppController {
  ...
  Info(@Req() req: Request) {
    ...
    const info = {
      promptImage2: `async function pasteImage(){try{const items=await navigator.clipboard.read();for(const item of items){if(item.types.includes("image/png")){const blob=await item.getType("image/png");const reader=new FileReader();reader.onloadend=()=>{const base64=reader.result;fetch("${domain}/post",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:"Bạn đang làm câu hỏi trắc nghiệm từ ảnh, hãy chọn đáp án đúng và chỉ cần đưa ra đáp án đúng A,B,C hoặc D, không cần giải thích:",image:base64})}).then((res)=>res.json()).then(data=>{console.log(data);alert(data.data)})};reader.readAsDataURL(blob)}}}catch(err){console.error("Clipboard error:",err)}}document.addEventListener('keydown',(e)=>{if(e.key.toLowerCase()==='p'||e.key.toLocaleLowerCase()==='y'){console.log(e.key);pasteImage()}});`,
      ...
      promptImage: `async function pasteImage(){try{const items=await navigator.clipboard.read();for(const item of items){if(item.types.includes("image/png")){const blob=await item.getType("image/png");const reader=new FileReader();reader.onloadend=()=>{const base64=reader.result;fetch("${domain}/post",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:"Bạn đang làm câu hỏi trắc nghiệm từ ảnh, hãy chọn đáp án đúng và chỉ cần đưa ra đáp án đúng A,B,C hoặc D, không cần giải thích:",image:base64})}).then((res)=>res.json()).then(data=>{const p = document.createElement("div");p.innerText=data.data;p.style.position = "fixed";p.style.top = "20px";p.style.left = "20px";p.style.color = "#858080ff";p.style.maxWidth = "40px";p.style.overflow = "hidden";p.style.whiteSpace = "nowrap";p.style.textOverflow = "ellipsis";document.body.appendChild(p);setTimeout(() => p.remove(), 5000);})};reader.readAsDataURL(blob)}}}catch(err){console.error("Clipboard error:",err)}}document.addEventListener('keydown',(e)=>{if(e.key.toLowerCase()==='p'||e.key.toLocaleLowerCase()==='y'){console.log(e.key);pasteImage()}});`,
